...
public class PodcastsResource {
	...
	/**
	 * Adds a new resource (podcast) from the given json format (at least title and
	...
	public Response createPodcast(Podcast podcast) throws AppException {
		...
		return Response.status(Response.Status.CREATED)// 201
				.entity("A new podcast has been created").header("Location",
						"http://localhost:8080/event-fabrication/podcasts/" + String.valueOf(createPodcastId))
				.build();
	}

	/**
	 * Adds a new podcast (resource) from "form" (at least title and feed elements
	...
	@Produces({ MediaType.TEXT_HTML })
	public Response createPodcastFromApplicationFormURLencoded(@FormParam("title") String title,
		...
		return Response.status(Response.Status.CREATED)// 201
				.entity("A new podcast/resource has been created at /event-fabrication/podcasts/" + createPodcastid)
				.header("Location",
						"http://localhost:8080/event-fabrication/podcasts/" + String.valueOf(createPodcastid))
				.build();
	}
	...
	/**
	 * A list of resources (here podcasts) provided in json format will be added to
	 * the database.
	 * 
	 * @param podcasts
	 * @return
	...
	public List<Podcast> getPodcasts(@QueryParam("orderByInsertionDate") String orderByInsertionDate,
			@QueryParam("numberDaysToLookBack") Integer numberDaysToLookBack) throws IOException, AppException {
		List<Podcast> podcasts = podcastService.getPodcasts(orderByInsertionDate, numberDaysToLookBack);
		return podcasts;
	}
	...
	public Response getPodcastById(@PathParam("id") Long id, @QueryParam("detailed") boolean detailed)
		...
		return Response.status(200)
				.entity(podcastById,
						detailed ? new Annotation[] { PodcastDetailedView.Factory.get() } : new Annotation[0])
	...
	// @Produces({ MediaType.APPLICATION_JSON, MediaType.APPLICATION_XML })
	// @PodcastDetailedView
	// public Podcast getPodcastById(@PathParam("id") Long id,
	...
	// return podcastById;
	//// return Response.status(200)
	//// .entity(podcastById, detailed ? new
	// Annotation[]{PodcastDetailedView.Factory.get()} : new Annotation[0])
	 ...
	 * is no resource yet at the specified location, then a podcast creation is
	...
	@Consumes({ MediaType.APPLICATION_JSON })
	@Produces({ MediaType.TEXT_HTML })
	public Response putPodcastById(@PathParam("id") Long id, Podcast podcast) throws AppException {
		...
		if (podcastById == null) {
			...
			return Response.status(Response.Status.OK)
					...
					.header("Location", "http://localhost:8080/event-fabrication/podcasts/" + String.valueOf(id))
					.build();
		}
	}
	...
	public Response partialUpdatePodcast(@PathParam("id") Long id, Podcast podcast) throws AppException {
		...
		return Response.status(Response.Status.OK)
				// 200
				.entity("The podcast you specified has been successfully updated").build();
	}
